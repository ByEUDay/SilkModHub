...
- **前端**：Python + PyQt6，精美的深色「丝绸」主题界面（另附暗夜/晴日两套主题）
- **后端**：C++（通过 pybind11 暴露给 Python），负责游戏路径检测、模组安装/卸载、
  以及对接雷霆商店 (Thunderstore) API 自动获取模组列表
...
- 🧵 **雷霆商店浏览**：一键拉取 `thunderstore.io/c/<community>` 社区的模组列表，
  支持关键字搜索、分类筛选（游戏内容 / Boss / UI / 辅助工具 / 翻译 / 音乐美术 / 依赖库）
- 📦 **本地模组管理**：安装、卸载、启用/禁用（禁用采用重命名 `.disabled` 后缀的方式，
  不删除文件，可随时恢复）
- 🎮 **游戏路径自动检测**：自动扫描 Steam 库（含 `libraryfolders.vdf` 自定义库路径）、
  GOG 常见路径；也可在设置中手动指定
- ⬇ **异步下载安装**：下载与解压在后台线程执行，带实时进度条，不卡界面
- 🎨 **三套主题**：丝绸（默认，胭脂红/墨黑，呼应游戏美术风格）、暗夜、晴日
...
代码中默认使用的社区标识是 `silksong`（对应 `https://thunderstore.io/c/silksong/`）。
...
- 模组依赖关系目前只做了数据采集（`TSPackage.dependencies`），尚未实现自动
  递归安装依赖包的逻辑——如果你需要，我可以在 `ModManager` / `Backend` 中补充这部分。
- 禁用模组通过重命名目录实现，具体是否被游戏内的模组加载器正确识别，取决于
  该加载器（如 BepInEx）的扫描逻辑，可能需要按实际加载器再做适配。
- 未内置代码签名/自动更新机制，如需打包为独立可执行文件，建议后续接入
  PyInstaller（前端）+ 静态链接（C++ 后端）。
